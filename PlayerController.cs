...
public class PlayerController : MonoBehaviour {
		...
		if (!die) {     //stop doing anything when player dead - 
            ...
            if (!isJumpHold )
            {
                Player.position = Vector3.Lerp(Player.position, 
                    jumpPlayer, Time.deltaTime*jumpForce);
            }
            else if(isJumpHold )
            {
                Player.position = Vector3.Lerp(Player.position,
                    jumpOffPlayer, Time.deltaTime * jumpForce);
            }
	...
	void OnTriggerEnter2D(Collider2D other){
        ...
        if (other.tag == "Dead-Invisible")
        {
            Slide(true);
        }
        else if(other.tag=="falseSlide")
        {
            Slide(false);
            Jump();
        }
